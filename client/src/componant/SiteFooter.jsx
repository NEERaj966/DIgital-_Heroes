      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid gap-6 lg:grid-cols-[1.15fr_1fr]">
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-6">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-300">Digital Heroes</p>
            <h2 className="mt-3 text-2xl font-semibold text-white">Charity golf, subscription access, monthly draws, and winner verification in one platform.</h2>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {footerSections.map((section) => (
              <div key={section.title} className="rounded-[24px] border border-white/10 bg-white/5 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{section.title}</p>
